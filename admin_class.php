...
Class Action {
	...
	function connect_user(){
		...
		$check = $this->db->query("SELECT * FROM candidate_referee_link where user_id ='{$_SESSION['login_id']}' ".(!empty($id) ? " and candidate_id = {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}

		$connect[] = $this->db->query("INSERT INTO candidate_referee_link set $data");
