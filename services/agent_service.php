...
/* ONLY ALLOW A USER THAT IS AGENT TO ACCESS THIS PAGE */
...
echo 'this is the session candidate id: ' . $_SESSION["agent_id"];
$tempagent_id = $_SESSION["agent_id"];
$tempemail = $_SESSION["email"];
$tempname = $_SESSION["fname"];
$templastname = $_SESSION["lname"];
echo 'Temporary email ' . $tempemail;

/* CANDIDATE EMAIL INVITE */
...
if (isset($_POST["submit_invite"])) {
    ...
    $actual_link = "http://www.onereference.net/candidatesignup.php?email=" . $email;
    //candidate email
    $toEmail = $tmp_email;

    echo ' agent email ' . $email;
    echo ' candidate email ' . $toEmail;
    $subject = "Signup and complete candidate profile ";
    ...
    $message = "
                    <html>
                    <head>
                    <title> Onereference : Invitation to to create a candidate profile  </title>
                    </head>
                    <body>
                    <p> Good Day, </p>
                    <p> An agent " . $tempname . " " . $templastname . " ,has invited you to join one reference to enable them to do a referral check.  </p>
                    <p> Follow the link below to register as a candidate and enable the agent to complete the job referal checks. </p>
                    <table>
                    <tr>
    ...
    /* Capture information in candidate invite table - this is when the agent invites the candidates */
    $query = "INSERT INTO candidate_invite
            	(agent_id, agent_name, agent_surname, agent_email, cand_email, cand_name, cand_surname, cand_contact, referal_date, invite_status)
            	VALUES ('$tempagent_id', '$tempname','$templastname','$tempemail','$tmp_email','$tmp_name','$tmp_surname','$tmp_contact_num',now(),'invite_send')";
...
if (isset($_POST["submit"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $query = "SELECT email FROM users WHERE rs_email='$email' LIMIT 1 ;";
    ...
    if (!$error) {
        ...
        if (!empty($tmp_role)) {
            if ($role_tmp == 'referree') {
                $referree_id = $today . $lname . $role . $random;
        ...
        $query1 = "INSERT INTO refereeprofile
			...
			'contact_num', 'ip_address', 'candidate_email','candidate_id', 'referree_id', 'pass_hash','account_status','account_type')
			
			('$tmp_username','$tmp_email','$tmp_password',NOW(),'$tmp_name', '$tmp_surnname','$tmp_company','$tmp_jobrole','$tmp_industry',
			'$tmp_contact_num','$ip','$candidate_email','$candidate_id','$tmp_username','$p_hash','$account_status','$tmp_userrole');";
