		...
		<div class="card-body">
			<form action="" id="manage_user">
					...
					<div class="col-md-6">
						...
						<div class="form-group">
							...
							<select name="type" id="type" class="custom-select custom-select-sm">
								<option value="3" <?php echo isset($type) && $type == 3 ? 'selected' : '' ?>>Referee</option>
								<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Candidate</option>
								<option value="1" <?php echo isset($type) && $type == 1 ? 'selected' : '' ?>>Agent</option>
							</select>
						</div>
						<?php else: ?>
				...
				<div class="col-lg-12 text-right justify-content-center d-flex">
					...
					<button class="btn btn-secondary" type="button" onclick="location.href = 'home.php?page=user_list'">Cancel</button>
				</div>
			</form>
		</div>
	</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.replace('home.php?page=user_list')
					},750)
				}else if(resp == 2){
